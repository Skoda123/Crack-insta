...
bad=0
def logo():
    print(f'\n\033[0m- \033[0m\033[41mBy : @maho_s9 && maho9s\033[0m\n- ')
    
G = '\033[1;32m' #اخضر
R = '\033[1;31m' # احمر
id = input(f'{G} ايدي حسابك : ')
os.system('clear')
token = input(f'{R} توكن بوتك : ')
os.system('clear')
...
def ahmed():
 ...
 ws=websocket.ahmed_connection("wss://51.79.208.190/Auth", header=headers, sslopt={"cert_reqs": ssl.CERT_NONE})
 ws.send(json.dumps(mahos))
 result=ws.recv()
 decmahos = gzip.decompress(result)
 print(G+str(decmahos))
 if '"comment":"Exists"' in str(decmahos):
  bad+=1
 elif '"status":"Success"' in str(decmahos):
  hits+=1
  tlg = f'''{user}
password >> hhhh
BY : @maho_s9
'''  
 ...
 elif '"comment":"Retry"' in str(decmahos):
  bad+=1
 else:
  print(decmahos)
executor=concurrent.futures.ThreadPoolExecutor(max_workers=500)
while True:
 executor.submit(ahmed)
 os.system('clear')
 logo()
 print(G+'Hits : '+str(hits))
 print(R+'Bad : '+str(bad))
